  ...
  { key: "no", header: "글번호", width: "w-24" },
  {
    ...
    render: (row) => (
      <div className="flex items-center gap-2">
        ...
        {row.important && <span className="font-extrabold text-red-500">중요</span>}
      </div>
    ),
  },
...
export default function NoticeListPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setIsLoading(true);
      try {
        const list = await fetchNoticeList({ keyword: searchTerm, page, size: pageSize });
        if (cancelled) return;
        ...
        if (cancelled) return;
        const message = error instanceof Error ? error.message : "공지사항 목록을 불러오지 못했습니다.";
        toast.error(message);
        setItems([]);
        setTotalPages(1);
        setTotalElements(0);
    ...
    <>
      ...
      <form className="mb-4 flex items-center gap-2" onSubmit={handleSearch}>
        <input
          ...
          className="h-10 flex-1 rounded border border-[var(--color-gray-200)] px-3 py-2"
        />
      ...
      <DataTable
        ...
        rowClassName={(row) => (row.important ? "bg-[var(--color-danger-10)] hover:bg-[var(--color-danger-20)]" : "")}
      ...
      {!isLoading && rows.length === 0 && (
        <p className="mt-4 text-sm text-[var(--color-gray-500)]">등록된 공지사항이 없습니다.</p>
      )}
